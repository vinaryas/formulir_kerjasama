...
@section('content')
<form class="card" action="{{ route('lingkup.store') }}" method="POST">
    {{ csrf_field() }}
    <div class="card-body">
        <div class="row">
            <div class="col-md-12">
                <label>Lingkup Kerjasama</label>
                <input type="text" id="lingkup" name="lingkup" class="form-control" required>
            </div>
        </div>
        <br>
        ...
        <table class="table table-bordered table-striped" id="table" style="width: 100%;">
            ...
            <tbody>
                @foreach ($lingkupKerjasama as $lingkup)
                    <tr>
                        <td>{{ $lingkup->id }}</td>
                        <td>{{ $lingkup->lingkup }}</td>
                        <td><a href="{{ route('lingkup.detail', $lingkup->id) }}"
